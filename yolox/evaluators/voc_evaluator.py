...
class VOCEvaluator:
    ...
    def evaluate(
        ...
        if trt_file is not None:
            ...
            model_trt = TRTModule()
            model_trt.set_state_dict(paddle.load(trt_file))

            x = paddle.ones([1, 3, test_size[0], test_size[1]])
            model(x)
            model = model_trt
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with paddle.no_grad():
                imgs = imgs.astype(paddle.float32)
        ...
        statistics = paddle.zeros([inference_time, nms_time, n_samples])
        if distributed:
            data_list = gather(data_list, dst=0)
            data_list = ChainMap(*data_list)
            paddle.distributed.reduce(statistics, dst=0)
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        for img_num in range(self.num_images):
            ...
            for j in range(self.num_classes):
                ...
                c_dets = paddle.concat((bboxes, scores.unsqueeze(1)), axis=1)
