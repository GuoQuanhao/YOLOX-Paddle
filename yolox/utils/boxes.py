...
import paddle
from .nms_utils import nms, batched_nms
...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        class_conf, class_pred = np.amax(image_pred[:, 5: 5 + num_classes], 1, keepdims=True), np.argmax(image_pred[:, 5: 5 + num_classes], 1)

        conf_mask = (image_pred[:, 4] * class_conf.squeeze() >= conf_thre).squeeze()
        # Detections ordered as (x1, y1, x2, y2, obj_conf, class_conf, class_pred)
        detections = np.concatenate((image_pred[:, :5], class_conf, class_pred.astype('float32')[..., None]), 1)
