...
from .profile_utils import paddle_profile
...
def get_model_info(model, tsize):
    ...
    flops, params = paddle_profile(deepcopy(model), inputs=(img,), verbose=False)
    params /= 1e6
    flops /= 1e9
...
def fuse_conv_and_bn(conv, bn):
    ...
    fusedconv.bias.set_value(paddle.mm(w_bn, b_conv.reshape([-1, 1])).reshape([-1]) + b_bn)
