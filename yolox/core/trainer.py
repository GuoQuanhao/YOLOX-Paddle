...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.scaler = paddle.amp.GradScaler(enable=args.fp16)
    ...
    def train_one_iter(self):
        ...
        # np.save('inps.npy', inps.cpu().numpy())
        # np.save('targets.npy', targets.cpu().numpy())
        # inps, targets = paddle.to_tensor(np.load('inps.npy')), paddle.to_tensor(np.load('targets.npy'))
        # self.model.set_state_dict(paddle.load('guding.pdparams'))
    ...
    def before_train(self):
        ...
        # Tensorboard logger
        if self.rank == 0:
            self.vdlogger = LogWriter(self.file_name)
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            self.vdlogger.add_scalar("val/COCOAP50", ap50, self.epoch + 1)
            self.vdlogger.add_scalar("val/COCOAP50_95", ap50_95, self.epoch + 1)
            logger.info("\n" + summary)
